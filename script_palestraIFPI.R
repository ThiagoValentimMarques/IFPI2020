...
#Pacote para ler arquivos do Excel
library(tidyverse)

#Local da pasta
caminho <- "C:\\Users\\Thiago\\Desktop\\Palestra - IFPI\\linhadotempo.csv" 
...
dados <- read.csv("COVIDBR_23jun2020.csv",header=TRUE,sep=";")
...
ne[,19]<-rep(seq(as.Date("25/02/2020",format="%d/%m/%y"),
             as.Date("23/06/2020",format="%d/%m/%y"),
             by=1),9)
names(ne)[19] <- "datapt"
...
hone <- "23/06" #Informar a data de hoje dd/mm
...
p3<-ne %>%
  ...
  scale_x_date(date_breaks = "1 week",date_labels = "%d/%b")+
  labs(x = "Data", y = "Casos confirmados", colour = "Estado",
       title="Acumulado de casos no Piauí",
       caption=" ") +
  theme(legend.position="none", 
        axis.text.y = element_text(vjust=0.8, size = 12),
        axis.text.x = element_text(vjust=1, size = 10,angle=60,hjust=1),
...
p4<-ne %>%
  ...
  scale_x_date(date_breaks = "1 week",date_labels = "%d/%b")+
  labs(x = "Data", y = "Óbitos confirmados", colour = "Estado",
       title="Acumulado de óbitos no Piauí",
       caption=" ") +
  theme(legend.position="none", 
        axis.text.y = element_text(vjust=0.8, size = 12),
        axis.text.x = element_text(vjust=1, size = 10,angle=60,hjust=1),
...
PI_mapa <- dados %>%
  filter(estado == "PI",codmun!=240000)%>%
  mutate(data = substr(data,1,5))%>%
  filter(data=="23/06") #Substituir por 11/04, 16/05 e 23/06

PI_mapa <- PI_mapa[-1,]

mapa <- get_brmap("City",geo.filter = list(State = 22))
mapa2 <- get_brmap("Region")

names(PI_mapa)[5]<-"City"

mapa<- mapa %>%
  mutate(City = as.numeric(substr(as.character(mapa$City),1,6)))

geral2 <- merge(x = mapa, y = PI_mapa, by = "City", all.x=TRUE)
...
geral2 <- geral2%>%
 ...
 # scale_fill_gradient(low="lightyellow", high="red3",
  ...
  coord_sf(xlim = c(-46, -40.5), ylim = c(-11, -2.5))
  
#Salvar em pdf 5 x 3 inches
#Salvar em pdf 5 x 4.2 inches (data mais recente)
...
########## Mapa 2 (óbitos)
  ...
  geral3 <- merge(x = mapa, y = PI_mapa, by = "City", all.x=TRUE)
  ...
  geral3 <- geral3%>%
    mutate(categ = cut(obitosAcumulado, c(-1,0,1, 10, 20, 50,291)))%>%
...
### a - Incidência
  ...
  a<-ggplot(geral2) + geom_sf(data=mapa2,fill = "gray70")+
      geom_sf(aes(fill =inc),size=0.1)+
      theme(panel.background = 
              element_rect(fill = "lightblue"),
            panel.grid = element_line(colour = "transparent"),
...
### b- mortalidade
  ...
  b<-ggplot(geral3) + geom_sf(data=mapa2,fill = "gray70")+
    geom_sf(aes(fill =mort),size=0.1)+
    theme(panel.background = 
            element_rect(fill = "lightblue"),
          panel.grid = element_line(colour = "transparent"),
          legend.position = "bottom",
